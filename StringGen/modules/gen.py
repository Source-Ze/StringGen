...
async def gen_session(
    ...
    if telethon:
        ...
        ty = f"🎙 بـايـࢪوجـࢪام v2 🎙"

    await message.reply_text(f"»» ⚡ ¦ بـدء إنـشـاء جـلسـة  {ty} ...")

    try:
        api_id = await Anony.ask(
            identifier=(message.chat.id, user_id, None),
            text="» 🎮حسنـا قم بأرسال الـ API_ID",
    ...
    try:
        api_hash = await Anony.ask(
            identifier=(message.chat.id, user_id, None),
            text="» 🎮حسنـا قم بأرسال الـ ᴀᴩɪ_ʜᴀsʜ",
            filters=filters.text,
            timeout=300,
        )
    except ListenerTimeout:
    ...
    try:
        phone_number = await Anony.ask(
            identifier=(message.chat.id, user_id, None),
            text="» ✔️الان ارسل رقمك مع رمز دولتك , مثال :+2010******** :",
            filters=filters.text,
            timeout=300,
        )
    except ListenerTimeout:
    ...
    await Anony.send_message(user_id, "»⬇️انتـظر لـحظـه سـوف نـرسـل كـود لحسابـك بالتليجـرام....")
    if telethon:
    ...
    try:
        otp = await Anony.ask(
            identifier=(message.chat.id, user_id, None),
            text=f"»»[شـوف آلصـورهہ‏‏ الكود مبعوت ازاي وابعت زيه عشان ميجيش ليك.ابطاء](https://telegra.ph/file/da1af082c6b754959ab47.jpg)» 🔍من فضلك افحص حسابك بالتليجرام وتفقد الكود من حساب اشعارات التليجرام. إذا كان\n  هناك تحقق بخطوتين( المرور ) ، أرسل كلمة المرور هنا بعد ارسال كود الدخول بالتنسيق أدناه.- اذا كانت كلمة المرور او الكود  هي\n 12345 يرجى ارسالها بالشكل التالي 1 2 3 4 5 مع وجود مسـافـات بين الارقام اذا احتجت مساعدة @UP_UO",
            filters=filters.text,
            timeout=600,
        )
    ...
    try:
        ...
        if telethon:
            ...
            await client(JoinChannelRequest("@UI_XB"))
        else:
